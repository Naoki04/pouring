...
#    with AsyncRobot(SyncRobot(RTDEController(ip='127.0.0.1'))) as robot:
        ...
        global trajectory
        while(trajectory.shape[0] > 0):
            target = trajectory[0,:]
            print("target:",target)
            trajectory = np.delete(trajectory, 0, 0)
            robot.move_linear((target[0], 0, target[1], 0, 0, 10))
            
        
        # Move backward and forward
        print("Moving backward and forward ...")
        robot.move_linear((50, 0, 0, 5, 0, 0))
        print("aaa")
        robot.move_linear((0, 20, 100, -10, 0, 0))
        print("bbb")
        robot.move_linear((0, 10, 0, 5, 0, 0))
        print("ccc")
